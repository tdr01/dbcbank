...
{defineLayout(); defineLayout1(); defineLayout2(); createTopMenu(); createTopMenu1(); createTopMenu2();  prepareListStyles(); prepareListStyles1(); prepareListStyles2(); setLEVELs(); setLEVELs1(); setLEVELs2(); Icons(); Icons1(); Icons2(); attachEventhandlers(); attachEventhandlers1(); attachEventhandlers2();}
...
function defineLayout1()
{
// Set Page position
document.getElementById("MDME1").style.position="static";
document.getElementById('MDME1').style.left= PagePositionLEFT+"px";
document.getElementById('MDME1').style.top= PagePositionTOP+"px";
document.getElementById('MDME1').style.zIndex=50;
}

function defineLayout2()
{
// Set Page position
document.getElementById("MDME2").style.position="static";
document.getElementById('MDME2').style.left= PagePositionLEFT+"px";
document.getElementById('MDME2').style.top= PagePositionTOP+"px";
document.getElementById('MDME2').style.zIndex=50;
}
...
function setLEVELs2()
{
ULCollection12=document.getElementById("MDME2").getElementsByTagName("UL");
ULCollection12.item(0).setAttribute("level", 1);

// Initally set all LI to level 1
LICollection=document.getElementById("MDME2").getElementsByTagName("LI");
for (a=0; a<LICollection.length;a++)
		{LICollection.item(a).setAttribute("level", 1);}

// Set all non-level 1 LI to respective levels
if (ULCollection2!=null)
	{
	for (u=0; u<ULCollection2.length; u++)
		{
		var ULChildrenCollection=ULCollection2.item(u).getElementsByTagName("UL");
		for (l=0; l<ULChildrenCollection.length; l++)
			{
			var previousLevel=parseInt(ULCollection2.item(u).getAttribute("level"));
			ULChildrenCollection.item(l).setAttribute("level", previousLevel+1);
			var LIChildrenCollection=ULChildrenCollection.item(l).getElementsByTagName("LI");
			for (m=0; m<LIChildrenCollection.length; m++)
				{LIChildrenCollection.item(m).setAttribute("level", previousLevel+1);}
...
if(showECOption=='yes' && oneBranch!='yes')
	...
	str+='<TABLE border="0" cellpadding="2" cellspacing="0" class="topMenu">';
	str+='<TR>';
	str+='<TD id="expandAllMenuItem2" onClick="ECALL2(1)">';
	if(imageEXPANDALL=="")
		{str+='Expand ALL<\/TD>';	}	
	else
		{str+='<IMG border="0" src="images\/'+imageEXPANDALL+'" alt="Expand ALL"><\/TD>';}
	str+='<TD id="collapseAllMenuItem2" onClick="ECALL2(0)">';
	if(imageCOLLAPSEALL=="")
		{str+='Collapse ALL<\/TD>';}	
	else	
		{str+='<IMG border="0" src="images\/'+imageCOLLAPSEALL+'" alt="Collapse ALL"><\/TD>';}
	str+='<\/TR>';
	str+='<\/TABLE>';
	document.getElementById("MDME2").insertAdjacentHTML("afterBegin", str);

	if(GlobalECState2==0)
		{document.getElementById("expandAllMenuItem2").style.display='inline'; 
		document.getElementById("collapseAllMenuItem2").style.display='none';}
	else
		{document.getElementById("expandAllMenuItem2").style.display='none'; 
		document.getElementById("collapseAllMenuItem2").style.display='inline'}
	}
}
...
if (ULCollection1!=null)
	{for (u=0; u<ULCollection1.length; u++)
		{ULCollection1.item(u).style.listStyleType="none";
		 ULCollection1.item(u).setAttribute("id", "BULID"+u);}}
}

function prepareListStyles2()
{
ULCollection2=document.getElementById("MDME2").getElementsByTagName("UL");
if (ULCollection2!=null)
	{for (u=0; u<ULCollection2.length; u++)
		{ULCollection2.item(u).style.listStyleType="none";
		 ULCollection2.item(u).setAttribute("id", "eULID"+u);}}
}
...
if (LICollection!=null)
	{	for (l=0; l<LICollection.length; l++)
			{LICollection.item(l).onmouseup=onMouseUpHandler2;}
	}
...
if(navigator.appName == 'Microsoft Internet Explorer')
	{
	document.getElementById('MDME2').style.filter="progid:DXImageTransform.Microsoft.Alpha(opacity="+TValue+")"
	}
else
	{
	document.getElementById('MDME2').style.MozOpacity=1;
	TValue=parseFloat(TValue/100-.001); // .001 is fix for moz opacity/image bug
	document.getElementById('MDME2').style.MozOpacity=TValue;}
}
...
function ECALL2(o) // Expand or Collapse "ALL" routine
{
//strip all images
LICollection=document.getElementById("MDME2").getElementsByTagName("LI");
	if (LICollection!=null)
...
firstLoad2=0; GlobalECState2=o; Icons2();
if(showECOption=='yes' && oneBranch!='yes')
	{
	if(GlobalECState2==0)
		{document.getElementById("expandAllMenuItem2").style.display='inline'; 
		...
		if(ULChildrenCol.length>0)
			...
			if(firstLoad2==0)
				...
				if(GlobalECState2==0)
					...
					LICollection.item(i).setAttribute("ECState",0);
					if(imagePLUS2!='')
						{	LICollection.item(i).style.background='url(images/'+imagePLUS2+') right top no-repeat';}
					LICollection.item(i).style.cursor="pointer";
					FirstULWithinLI_ELEMENT.item(0).style.display='none';
					}
				else
					...
					LICollection.item(i).setAttribute("ECState",1);
					if(imageMINUS2!='')
						{	LICollection.item(i).style.background='url(images/'+imageMINUS2+') right top no-repeat';}
					LICollection.item(i).style.cursor="pointer";
					FirstULWithinLI_ELEMENT.item(0).style.display='block';
					}
				}
							...
							for (tar=0;tar<tarObjParentLICol.length; tar++)
								...
								if(targetLevel==ItemLevel)
									...
									if(tarObjParentLIULCol.length!=0 && tarObj!=tarObjParentLICol.item(tar))
										...
										tarObjParentLIULCol.item(0).style.display='none';
										if(imagePLUS2!='')
											{	tarObjParentLICol.item(tar).style.background='url(images/'+imagePLUS2+') right top no-repeat';
												tarObjParentLICol.item(tar).style.listStylePosition='inside';
												tarObjParentLICol.item(tar).setAttribute("ECState",0);}
										}
									}
...
if(firstLoad2==0){firstLoad2=1;}
if(showECOption=='yes' && oneBranch!='yes')
	{	document.getElementById('expandAllMenuItem2').style.cursor="pointer";
		document.getElementById('collapseAllMenuItem2').style.cursor="pointer";}
}
